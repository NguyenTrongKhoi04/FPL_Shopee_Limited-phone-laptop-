...
<div class="container-fluid">
    ...
    <div class="float-end mb-3">
        <button type="button" class="btn btn-success " data-bs-toggle="modal" data-bs-target="#exampleModal">Thêm mới
            sản phẩm</button>
    ...
    <div id="demo" class="collapse mb-3" style="margin-top: 80px; margin-left: 900px;">
        <button type="button" class="btn btn-info " data-bs-toggle="modal" data-bs-target="#danhsachdanhmuc">Danh sách
            danh mục</button>
        <button type="button" class="btn btn-info " data-bs-toggle="modal" data-bs-target="#exampleModal2">Thêm mới danh
            mục</button>
    </div>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @endif
                                @endforeach
                                <td><a href="{{route('detailSubcate/'.$sub->id)}}"><button type="button"
                                            class="btn btn-warning">Update</button></a> | <a
                                        onclick="confirm('bạn có chắc xóa danh mục này')"
                                        href="{{route('deleteSubcate/'.$sub->id)}}"><button
    ...
    <table class="table table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <button class="btn btn-warning" style="font-size: 10px;" data-bs-toggle="modal"
                        data-bs-target="#updateProduct{{$p->id}}">
                        Update
                    </button> <br>
                    <a href="{{route('deleteStoreProduct/'.$p->id)}}"> <button onclick="confirm('bạn có chắc muốn xóa')"
            ...
            <div class=" modal fade" id="updateProduct{{$p->id}}" tabindex="-1" aria-labelledby="exampleModalLabel"
                            aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h1 class="modal-title fs-5" id="exampleModalLabel"><b>Edit</b> {{$p->name_pro}}
                                        </h1>
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('edit-storeProduct/'.$p->id)}}" method="post">
                                            <div class="mb-3">
                                                <label for="exampleInputEmail1" class="form-label">Tên sản phẩm</label>
                                                <input type="text" name="name_pro" class="form-control" id=""
                                                    value="{{$p->name_pro}}">
                                            </div>
                                            <div class="mb-3">
                                                <label for="exampleInputPassword1" class="form-label">Số lượng còn
                                                    lại</label>
                                                <input type="number" name="quantity" class="form-control" id=""
                                                    value="{{$p->quantity}}">
                                            </div>
                                            <div class="mb-3">
                                                <label for="exampleInputPassword1" class="form-label">Loại sản
                                                    phẩm</label>
                                                <select class="rounded " name="id_subcategory" id="">
                                                    @foreach($subcategory as $sub)
                                                    <option class="rounded " value="{{$sub->id}}"
                                                        <?= ($sub->id == $p->id_subcategory) ? 'selected' : ""  ?>>
                                                        {{$sub->name_subcate}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="mb-3">
